//내가했던 연산을 축약해서 쓸수있도록 해보자
//카운트 값에다가 뭔가 매개변수를 줘서 if같은걸로 처리를 해버린다면 그건 범용성이 있는 고차함수가 아니다.
//카운트에는 제약을 주면 안된다. 그래서 클로저를 쓰는거다. 클로저는 범용성이 있는 고차원함수를 만들기 위해 쓰이는것이다.
/*
const closure = counter();

//증감연산 함수를 굳이 만들지 않고 화살표 함수로 표현
closure(number => ++number); //플러스한 값이 반환됨
closure(number => --number); //마이너스한 값이 반환됨

즉시호출함수를 써서 활용할수도 있다.
즉시함수를 써서 바로 클로저를 불러온다.
...
이맇게하면 아래와 같은 과정이 필요없다. 
const 연산공장 = counter();

바로 클로저를 호출하면서 콜백만 넘겨주면되니까 사용자 입장에서는 훨씬 편하게 사용가능하다.
